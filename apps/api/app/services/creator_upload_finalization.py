...
class CreatorUploadFinalizer:
    ...
    transaction. A verified private object is retained after a transient
    persistence failure so the next call can resume through the storage
    adapter's ``NoSuchUpload`` verification path. Rights denial is terminal and
    triggers object deletion plus a durable session/job abort. Unknown database
    commit state never deletes bytes that may already be referenced.
    """
