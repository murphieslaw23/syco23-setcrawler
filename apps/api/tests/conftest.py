...
@pytest.fixture(autouse=True)
def current_postgres_creator_upload_clock(
    request: pytest.FixtureRequest,
    monkeypatch: pytest.MonkeyPatch,
) -> Iterator[None]:
    """Keep wall-clock PostgreSQL upload tests aligned with DB trigger time.

    These two integration tests exercise triggers whose ``updated_at`` value
    is generated by PostgreSQL's real clock. Their module-level FIXED_NOW was
    useful while the dates were in the future, but eventually made otherwise
    valid sessions look expired. Only the PostgreSQL cases need a live anchor;
    deterministic in-memory tests continue to use their frozen clock.
    """

    postgres_clock_tests = {
        (
            "test_creator_upload_postgres",
            "test_postgres_creator_upload_session_enforces_database_fences",
        ),
        (
            "test_creator_upload_transactions",
            "test_postgres_creator_upload_completion_is_one_transaction",
        ),
    }
    if (request.module.__name__, request.node.name) not in postgres_clock_tests:
        yield
        return

    monkeypatch.setattr(request.module, "FIXED_NOW", datetime.now(UTC))
    yield
